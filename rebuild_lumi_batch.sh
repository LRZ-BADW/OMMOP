...
make
echo -e '#!/bin/bash'"\nenv | grep ^OMP_; ${MODULESCMD} make t b;" > lumi_tmp_run.sh
chmod +x lumi_tmp_run.sh
srun --exclude=nid005079 --partition=eap --account=${SLURM_JOB_ACCOUNT} --time=10:00 --ntasks=1 --cpus-per-task=1 --gpus-per-task=1 --gpus=1 lumi_tmp_run.sh | tee lumi_tmp_run_`date +%Y%m%d_%H%M`.log
